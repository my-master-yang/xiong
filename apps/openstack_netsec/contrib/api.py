...
def init_notifier(connection_str, host="localhost"):
    _notifier = notifier.create(connection_str, project='horizon', service='horizon', host=host)
...
def _get_engine(request):
    connection_str = PROFILER_SETTINGS.get('receiver_connection_string', "mongodb://")
...
def list_traces(request):
    ...
    return [{
        ...
        'origin': trace['info']['request']['path']
    } for trace in traces]
...
def update_trace_headers(keys, **kwargs):
    ...
    trace_info = utils.signed_unpack(trace_headers[web.X_TRACE_INFO], trace_headers[web.X_TRACE_HMAC], keys)
    trace_info.update(kwargs)
    p = profiler.get()
    trace_data = utils.signed_pack(trace_info, p.hmac_key)
    return json.dumps({web.X_TRACE_INFO: trace_data[0], web.X_TRACE_HMAC: trace_data[1]})
