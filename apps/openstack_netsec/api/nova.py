...
from openstack_netsec.horizon import exceptions as horizon_exceptions
...
def get_microversion(request, feature):
    ...
    return (microversions.get_microversion_for_feature('nova', feature, api_versions.APIVersion, min_ver, max_ver))
...
class Server(base.APIResourceWrapper):
    ...
    """
    _attrs = [
        'addresses', 'attrs', 'id', 'image', 'links', 'description', 'metadata', 'name', 'private_ip', 'public_ip',
        'status', 'uuid', 'image_name', 'VirtualInterfaces', 'flavor', 'key_name', 'fault', 'tenant_id', 'user_id',
        'created', 'locked', 'OS-EXT-STS:power_state', 'OS-EXT-STS:task_state', 'OS-EXT-SRV-ATTR:instance_name',
        'OS-EXT-SRV-ATTR:host', 'OS-EXT-AZ:availability_zone', 'OS-DCF:diskConfig'
    ]
    ...
    def image_name(self):
        ...
        from openstack_netsec.api import glance
        ...
        if 'name' in self.image:
            ...
            try:
                ...
                return image.name
            except (glance_exceptions.ClientException, horizon_exceptions.ServiceCatalogException):
...
class Hypervisor(base.APIDictWrapper):
    ...
    _attrs = ['manager', '_loaded', '_info', 'hypervisor_hostname', 'id', 'servers']
...
class NovaUsage(base.APIResourceWrapper):
    ...
    _attrs = [
        'start', 'server_usages', 'stop', 'tenant_id', 'total_local_gb_usage', 'total_memory_mb_usage',
        'total_vcpus_usage', 'total_hours'
    ]

    def get_summary(self):
        return {
            'instances': self.total_active_instances,
    ...
    @property
    def vcpus(self):
        return sum(s['vcpus'] for s in self.server_usages if s['ended_at'] is None)
    ...
    @property
    def local_gb(self):
        return sum(s['local_gb'] for s in self.server_usages if s['ended_at'] is None)

    @property
    def memory_mb(self):
        return sum(s['memory_mb'] for s in self.server_usages if s['ended_at'] is None)
...
def get_auth_params_from_request(request):
    ...
    """
    return (request.user.username, request.user.token.id, request.user.tenant_id,
            request.user.token.project.get('domain_id'), base.url_for(request, 'compute'),
...
def novaclient(request_auth_params, version=None):
    (username, token_id, project_id, project_domain_id, nova_url, auth_url) = request_auth_params
    if version is None:
...
def server_vnc_console(request, instance_id, console_type='novnc'):
    return VNCConsole(novaclient(request).servers.get_vnc_console(instance_id, console_type)['console'])


@profiler.trace
def server_spice_console(request, instance_id, console_type='spice-html5'):
    return SPICEConsole(novaclient(request).servers.get_spice_console(instance_id, console_type)['console'])


@profiler.trace
def server_rdp_console(request, instance_id, console_type='rdp-html5'):
    return RDPConsole(novaclient(request).servers.get_rdp_console(instance_id, console_type)['console'])


@profiler.trace
def server_serial_console(request, instance_id, console_type='serial'):
    return SerialConsole(novaclient(request).servers.get_serial_console(instance_id, console_type)['console'])
...
def flavor_create(request,
                  ...
                  is_public=True,
                  rxtx_factor=1):
    flavor = novaclient(request).flavors.create(
        name,
        memory,
        vcpu,
        disk,
...
@profiler.trace
def update_pagination(entities, page_size, marker, sort_dir, sort_key, reversed_order):
    ...
    if reversed_order:
        entities = sorted(
            entities, key=lambda entity: (getattr(entity, sort_key) or '').lower(), reverse=(sort_dir == 'asc'))
...
def flavor_list_paged(request,
    ...
    if paginate:
        ...
        flavors = novaclient(request).flavors.list(
            is_public=is_public, marker=marker, limit=page_size + 1, sort_key=sort_key, sort_dir=sort_dir)
        flavors, has_more_data, has_prev_data = update_pagination(flavors, page_size, marker, sort_dir, sort_key,
                                                                  reversed_order)
    else:
        flavors = novaclient(request).flavors.list(is_public=is_public)
...
def add_tenant_to_flavor(request, flavor, tenant):
    ...
    return novaclient(request).flavor_access.add_tenant_access(flavor=flavor, tenant=tenant)
...
def server_create(request,
    ...
    return Server(
        get_novaclient_with_instance_desc(request).servers.create(
            name.strip(),
            image,
            flavor,
            userdata=user_data,
...
def server_get(request, instance_id):
    return Server(get_novaclient_with_instance_desc(request).servers.get(instance_id), request)
...
def server_list(request, search_opts=None, detailed=True):
    ...
    servers = [Server(s, request) for s in nova_client.servers.list(detailed, search_opts)]
...
@profiler.trace
def server_rebuild(request, instance_id, image_id, password=None, disk_config=None):
    return novaclient(request).servers.rebuild(instance_id, image_id, password, disk_config)
...
@profiler.trace
def server_live_migrate(request, instance_id, host, block_migration=False, disk_over_commit=False):
    novaclient(request).servers.live_migrate(instance_id, host, block_migration, disk_over_commit)


@profiler.trace
def server_resize(request, instance_id, flavor, disk_config=None, **kwargs):
    novaclient(request).servers.resize(instance_id, flavor, disk_config, **kwargs)
...
def usage_list(request, start, end):
    ...
    if client.api_version >= api_versions.APIVersion('2.40'):
        ...
        while marker:
            next_usage_list = client.usage.list(start, end, True, marker=marker)
...
def instance_volume_attach(request, volume_id, instance_id, device):
    return novaclient(request).volumes.create_server_volume(instance_id, volume_id, device)


@profiler.trace
def instance_volume_detach(request, instance_id, att_id):
    return novaclient(request).volumes.delete_server_volume(instance_id, att_id)


@profiler.trace
def instance_volumes_list(request, instance_id):
    from openstack_netsec.api import cinder
...
@profiler.trace
def migrate_host(request, host, live_migrate=False, disk_over_commit=False, block_migration=False):
    hypervisors = novaclient(request).hypervisors.search(host, True)
    response = []
    err_code = None
            ...
            try:
                if live_migrate:
                    ...
                    if instance.status in ["ACTIVE", "PAUSED"]:
                        novaclient(request).servers.live_migrate(server['uuid'], None, block_migration,
                                                                 disk_over_commit)
                    else:
...
def service_disable(request, host, binary, reason=None):
    if reason:
        return novaclient(request).services.disable_log_reason(host, binary, reason)
    else:
...
@profiler.trace
def interface_attach(request, server, port_id=None, net_id=None, fixed_ip=None):
    return novaclient(request).servers.interface_attach(server, port_id, net_id, fixed_ip)
...
def list_extensions(nova_api):
    ...
    return tuple(
        extension for extension in nova_list_extensions.ListExtManager(nova_api).show_all()
...
def instance_action_list(request, instance_id):
    return nova_instance_action.InstanceActionManager(novaclient(request)).list(instance_id)
