...
from openstack_netsec.horizon import messages
...
from openstack_netsec import policy
...
ROUTER_INTERFACE_OWNERS = ('network:router_interface', 'network:router_interface_distributed',
...
class NeutronAPIDictWrapper(base.APIDictWrapper):
    def __init__(self, apidict):
        ...
        apidict.update({key.replace(':', '__'): value for key, value in apidict.items() if ':' in key})
    ...
    @property
    def name_or_id(self):
        return (self._apidict.get('name').strip() or '(%s)' % self._apidict['id'][:13])
...
class SecurityGroup(NeutronAPIDictWrapper):
    ...
    def __init__(self, sg, sg_dict=None):
        ...
        sg['rules'] = [SecurityGroupRule(rule, sg_dict) for rule in sg['security_group_rules']]
    ...
    def __str__(self):
        ...
        if self.from_port:
            if self.from_port == self.to_port:
                proto_port = ("%s/%s" % (self.from_port, self.ip_protocol.lower()))
            else:
                proto_port = ("%s-%s/%s" % (self.from_port, self.to_port, self.ip_protocol.lower()))
...
class SecurityGroupManager(object):
    ...
    * group: remote security group. The value should be a format of
    ...
    def _sg_name_dict(self, sg_id, rules):
        ...
        related_sgs = self.client.list_security_groups(id=related_ids, fields=['id', 'name'])
    ...
    @profiler.trace
    def update_instance_security_group(self, instance_id, new_security_group_ids):
...
class FloatingIp(base.APIDictWrapper):
    _attrs = ['id', 'ip', 'fixed_ip', 'port_id', 'instance_id', 'instance_type', 'pool']
...
class FloatingIpManager(object):
    ...
    * instance_id: Instance ID of an associated with the Floating IP
    ...
    def list_pools(self):
        ...
        return [FloatingIpPool(pool) for pool in self.client.list_networks(**search_opts).get('networks')]
    ...
    def _set_instance_info(self, fip, port=None):
        if fip['port_id']:
            ...
            fip['instance_type'] = self._get_instance_type_from_device_owner(port.device_owner)
        else:
    ...
    def _get_reachable_subnets(self, ports):
        if not is_enabled_by_config('enable_fip_topology_check', True):
            ...
            return set(p.fixed_ips[0]['subnet_id'] for p in ports if p.fixed_ips)
        # Retrieve subnet list reachable from external network
        ext_net_ids = [ext_net.id for ext_net in self.list_pools()]
        gw_routers = [
            r.id for r in router_list(self.request)
            if (r.external_gateway_info and r.external_gateway_info.get('network_id') in ext_net_ids)
        ]
        reachable_subnets = set([
            p.fixed_ips[0]['subnet_id'] for p in ports
            if ((p.device_owner in ROUTER_INTERFACE_OWNERS) and (p.device_id in gw_routers))
        ])
        ...
        shared = set([s.id for n in network_list(self.request, shared=True) for s in n.subnets])
    ...
    def list_targets(self):
        ...
        server_dict = collections.OrderedDict([(s.id, s.name) for s in servers])
        ...
        for p in ports:
            ...
            for ip in p.fixed_ips:
                ...
                if netaddr.IPAddress(ip['ip_address']).version != 4:
                    continue
                targets.append(FloatingIpTarget(p, ip['ip_address'], server_name))
        return targets
    ...
    def get_target_id_by_instance(self, instance_id, target_list=None):
        ...
        if target_list is not None:
            targets = [target for target in target_list if target['instance_id'] == instance_id]
            if not targets:
                return None
            return targets[0]['id']
        else:
            ...
            if not ports:
                return None
            return '{0}_{1}'.format(ports[0].id, ports[0].fixed_ips[0]['ip_address'])
    ...
    def list_target_id_by_instance(self, instance_id, target_list=None):
        ...
        """
        if target_list is not None:
            return [target['id'] for target in target_list if target['instance_id'] == instance_id]
        else:
            ports = self._target_ports_by_instance(instance_id)
            return ['{0}_{1}'.format(p.id, p.fixed_ips[0]['ip_address']) for p in ports]
...
@profiler.trace
def list_resources_with_long_filters(list_method, filter_attr, filter_values, **params):
    ...
    try:
        ...
        all_filter_len = sum(len(filter_attr) + len(val) + 2 for val in filter_values)
...
def network_list(request, **params):
    ...
    for n in networks:
        ...
        n['subnets'] = [subnet_dict[s] for s in n.get('subnets', []) if s in subnet_dict]
    return [Network(n) for n in networks]


@profiler.trace
def network_list_for_tenant(request, tenant_id, include_external=False, **params):
...
def network_get(request, network_id, expand_subnet=True, **params):
    LOG.debug("network_get(): netid=%(network_id)s, params=%(params)s", {'network_id': network_id, 'params': params})
    network = neutronclient(request).show_network(network_id, **params).get('network')
    if expand_subnet:
        ...
        try:
            ...
            # subnet_list() once.
            network['subnets'] = [subnet_get(request, sid) for sid in network['subnets']]
...
def network_update(request, network_id, **kwargs):
    ...
    body = {'network': kwargs}
    network = neutronclient(request).update_network(network_id, body=body).get('network')
    return Network(network)
...
def subnet_get(request, subnet_id, **params):
    LOG.debug("subnet_get(): subnetid=%(subnet_id)s, params=%(params)s", {'subnet_id': subnet_id, 'params': params})
    subnet = neutronclient(request).show_subnet(subnet_id, **params).get('subnet')
...
def subnet_create(request, network_id, **kwargs):
    ...
    result.
    """
    LOG.debug("subnet_create(): netid=%(network_id)s, kwargs=%(kwargs)s", {'network_id': network_id, 'kwargs': kwargs})
...
def subnet_update(request, subnet_id, **kwargs):
    LOG.debug("subnet_update(): subnetid=%(subnet_id)s, kwargs=%(kwargs)s", {'subnet_id': subnet_id, 'kwargs': kwargs})
    body = {'subnet': kwargs}
    subnet = neutronclient(request).update_subnet(subnet_id, body=body).get('subnet')
    return Subnet(subnet)
...
def port_list_with_trunk_types(request, **params):
    ...
    }) for t in trunks for s in t['sub_ports']])
...
def port_get(request, port_id, **params):
    LOG.debug("port_get(): portid=%(port_id)s, params=%(params)s", {'port_id': port_id, 'params': params})
...
def port_create(request, network_id, **kwargs):
    ...
    :returns: Port object
    """
    LOG.debug("port_create(): netid=%(network_id)s, kwargs=%(kwargs)s", {'network_id': network_id, 'kwargs': kwargs})
...
def port_update(request, port_id, **kwargs):
    LOG.debug("port_update(): portid=%(port_id)s, kwargs=%(kwargs)s", {'port_id': port_id, 'kwargs': kwargs})
    kwargs = unescape_port_kwargs(**kwargs)
    body = {'port': kwargs}
...
def router_update(request, r_id, **kwargs):
    LOG.debug("router_update(): router_id=%(r_id)s, kwargs=%(kwargs)s", {'r_id': r_id, 'kwargs': kwargs})
    body = {'router': {}}
    body['router'].update(kwargs)
...
def router_get(request, router_id, **params):
    router = neutronclient(request).show_router(router_id, **params).get('router')
    return Router(router)
...
def router_static_route_remove(request, router_id, route_ids):
    ...
    for oldroute in currentroutes:
        if oldroute.id not in route_ids:
            newroutes.append({'nexthop': oldroute.nexthop, 'destination': oldroute.destination})
...
def router_static_route_add(request, router_id, newroute):
    body = {}
    currentroutes = router_static_route_list(request, router_id=router_id)
    body['routes'] = [newroute] + [{'nexthop': r.nexthop, 'destination': r.destination} for r in currentroutes]
    new = router_update(request, router_id, **body)
    return new
...
def list_dhcp_agent_hosting_networks(request, network, **params):
    agents = neutronclient(request).list_dhcp_agent_hosting_networks(network, **params)
    return [Agent(a) for a in agents['agents']]


@profiler.trace
def list_l3_agent_hosting_router(request, router, **params):
    agents = neutronclient(request).list_l3_agent_hosting_routers(router, **params)
    return [Agent(a) for a in agents['agents']]


@profiler.trace
def show_network_ip_availability(request, network_id):
    ip_availability = neutronclient(request).show_network_ip_availability(network_id)
...
def remove_network_from_dhcp_agent(request, dhcp_agent, network_id):
    return neutronclient(request).remove_network_from_dhcp_agent(dhcp_agent, network_id)
...
def floating_ip_target_get_by_instance(request, instance_id, cache=None):
    return FloatingIpManager(request).get_target_id_by_instance(instance_id, cache)


def floating_ip_target_list_by_instance(request, instance_id, cache=None):
    return FloatingIpManager(request).list_target_id_by_instance(instance_id, cache)
...
def security_group_rule_create(request, parent_group_id, direction, ethertype, ip_protocol, from_port, to_port, cidr,
                               group_id):
    return SecurityGroupManager(request).rule_create(parent_group_id, direction, ethertype, ip_protocol, from_port,
...
def server_update_security_groups(request, instance_id, new_security_group_ids):
    return SecurityGroupManager(request).update_instance_security_group(instance_id, new_security_group_ids)


# TODO(pkarikh) need to uncomment when osprofiler will have no
# issues with unicode in:
# openstack_netsec/test/test_data/nova_data.py#L470 data
# @profiler.trace
def servers_update_addresses(request, servers, all_tenants=False):
    ...
    try:
        ...
        ports = list_resources_with_long_filters(
            port_list, 'device_id', tuple([instance.id for instance in servers]), request=request)
        fips = FloatingIpManager(request)
        if fips.is_supported():
            floating_ips = list_resources_with_long_filters(
                fips.list, 'port_id', tuple([port.id for port in ports]), all_tenants=all_tenants)
        else:
            floating_ips = []
        # NOTE(e0ne): we need frozenset here to work with @memoized decorator.
        # @memoized works with hashable arguments only
        networks = list_resources_with_long_filters(
            network_list, 'id', frozenset([port.network_id for port in ports]), request=request)
    ...
    for server in servers:
        try:
            addresses = _server_get_addresses(request, server, instances_ports, ports_floating_ips, network_names)
        except Exception as e:
...
def _server_get_addresses(request, server, ports, floating_ips, network_names):
    def _format_address(mac, ip, type):
        try:
            ...
            LOG.error('Unable to parse IP address %(ip)s: %(exc)s', {'ip': ip, 'exc': e})
            error_message = _('Unable to parse IP address %s.') % ip
            messages.error(request, error_message)
            raise
        return {u'OS-EXT-IPS-MAC:mac_addr': mac, u'version': version, u'addr': ip, u'OS-EXT-IPS:type': type}
    ...
    for port in instance_ports:
        ...
        if network_name is not None:
            for fixed_ip in port.fixed_ips:
                addresses[network_name].append(_format_address(port.mac_address, fixed_ip['ip_address'], u'fixed'))
            port_fips = floating_ips.get(port.id, [])
            for fip in port_fips:
                addresses[network_name].append(_format_address(port.mac_address, fip.floating_ip_address, u'floating'))
...
def is_service_enabled(request, config_name, ext_name):
    return (is_enabled_by_config(config_name) and is_extension_supported(request, ext_name))


@memoized
def is_quotas_extension_supported(request):
    return (is_enabled_by_config('enable_quotas', False) and is_extension_supported(request, 'quotas'))


@memoized
def is_router_enabled(request):
    return (is_enabled_by_config('enable_router') and is_extension_supported(request, 'router'))
...
FEATURE_MAP = {
    ...
    'ext-gw-mode': {
        ...
        'policies': {
            'create_router_enable_snat': 'create_router:external_gateway_info:enable_snat',
            'update_router_enable_snat': 'update_router:external_gateway_info:enable_snat',
        }
    },
}
...
def policy_list(request, **kwargs):
    ...
    policies = neutronclient(request).list_qos_policies(**kwargs).get('policies')
...
def policy_get(request, policy_id, **kwargs):
    ...
    policy = neutronclient(request).show_qos_policy(policy_id, **kwargs).get('policy')
    return QoSPolicy(policy)


@profiler.trace
def list_availability_zones(request, resource=None, state=None):
    az_list = neutronclient(request).list_availability_zones().get('availability_zones')
    if resource:
