...
class Service(base.APIDictWrapper):
    ...
    def __init__(self, service, region, *args, **kwargs):
        ...
        self.public_url = base.get_url_for_service(service, region, 'publicURL')
...
def keystoneclient(request, admin=False):
    ...
    if (hasattr(request, cache_attr)
            and (not user.token.id or getattr(request, cache_attr).auth_token == user.token.id)):
        ...
        conn = api_version['client'].Client(
            token=token_id,
            endpoint=endpoint,
...
def domain_create(request, name, description=None, enabled=None):
    ...
    return manager.create(name=name, description=description, enabled=enabled)
...
@profiler.trace
def domain_update(request, domain_id, name=None, description=None, enabled=None):
    manager = keystoneclient(request, admin=True).domains
    try:
        response = manager.update(domain_id, name=name, description=description, enabled=enabled)
    except Exception:
...
def is_domain_admin(request):
    return policy.check((("identity", "admin_and_matching_domain_id"), ), request)
    ...
    try:
        if VERSIONS.active < 3:
            ...
            return manager.update(
                project, name=name, description=description, enabled=enabled, domain=domain, **kwargs)
    except keystone_exceptions.Conflict:
        raise exceptions.Conflict()
...
def user_update_password(request, user, password, admin=True):
    ...
    if not keystone_can_edit_user():
        raise keystone_exceptions.ClientException(405, _("Identity service does not allow editing user password."))
...
def group_create(request, domain_id, name, description=None):
    ...
    return manager.create(domain=domain_id, name=name, description=description)
...
def group_list(request, domain=None, project=None, user=None, filters=None):
    manager = keystoneclient(request, admin=True).groups
    groups = []
    kwargs = {"domain": domain, "user": user, "name": None}
    if filters is not None:
        kwargs.update(filters)
    if 'id' in kwargs:
        try:
...
def group_update(request, group_id, name=None, description=None):
    manager = keystoneclient(request, admin=True).groups
    return manager.update(group=group_id, name=name, description=description)
...
@profiler.trace
def role_assignments_list(request,
                          project=None,
                          user=None,
                          role=None,
                          group=None,
...
def get_domain_users_roles(request, domain):
    ...
    domain_role_assignments = role_assignments_list(request, domain=domain, include_subtree=False)
    for role_assignment in domain_role_assignments:
        ...
        # filter by domain_id
        if ('domain' in role_assignment.scope and role_assignment.scope['domain']['id'] == domain):
...
def add_group_role(request, role, group, domain=None, project=None):
    ...
    manager = keystoneclient(request, admin=True).roles
    return manager.grant(role=role, group=group, domain=domain, project=project)
...
def remove_group_roles(request, group, domain=None, project=None):
    ...
    for role in roles:
        remove_group_role(request, role=role.id, group=group, domain=domain, project=project)
...
def keystone_can_edit_domain():
    ...
    multi_domain_support = getattr(settings, 'OPENSTACK_KEYSTONE_MULTIDOMAIN_SUPPORT', False)
...
def is_multi_domain_enabled():
    return (VERSIONS.active >= 3 and getattr(settings, 'OPENSTACK_KEYSTONE_MULTIDOMAIN_SUPPORT', False))
