...
def swift_get_containers(request, marker=None, prefix=None):
    ...
    headers, containers = swift_api(request).get_account(
        limit=limit + 1, marker=marker, prefix=prefix, full_listing=True)
...
def swift_get_container(request, container_name, with_data=True):
    ...
    try:
        ...
        if is_public:
            swift_endpoint = base.url_for(request, 'object-store', endpoint_type='publicURL')
...
@profiler.trace
def swift_get_objects(request, container_name, prefix=None, marker=None, limit=None):
    limit = limit or getattr(settings, 'API_RESULT_LIMIT', 1000)
    kwargs = dict(prefix=prefix, marker=marker, limit=limit + 1, delimiter=FOLDER_DELIMITER, full_listing=True)
    headers, objects = swift_api(request).get_container(container_name, **kwargs)
...
def swift_filter_objects(request, filter_string, container_name, prefix=None, marker=None):
    ...
    limit = 9999
    objects = swift_get_objects(request, container_name, prefix=prefix, marker=marker, limit=limit)
...
def swift_copy_object(request, orig_container_name, orig_object_name, new_container_name, new_object_name):
    ...
    headers = {"X-Copy-From": FOLDER_DELIMITER.join([orig_container_name, orig_object_name])}
    return swift_api(request).put_object(new_container_name, new_object_name, None, headers=headers)


@profiler.trace
def swift_upload_object(request, container_name, object_name, object_file=None):
    headers = {}
    size = 0
    if object_file:
    ...
    etag = swift_api(request).put_object(
        container_name, object_name, object_file, content_length=size, headers=headers)
...
def swift_create_pseudo_folder(request, container_name, pseudo_folder_name):
    ...
    headers = {}
    etag = swift_api(request).put_object(container_name, pseudo_folder_name, None, headers=headers)
    obj_info = {'name': pseudo_folder_name, 'etag': etag}
...
@profiler.trace
def swift_get_object(request, container_name, object_name, with_data=True, resp_chunk_size=CHUNK_SIZE):
    if with_data:
        headers, data = swift_api(request).get_object(container_name, object_name, resp_chunk_size=resp_chunk_size)
    else:
        data = None
