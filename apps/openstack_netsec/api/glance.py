...
try:
    ...
    VERSIONS.load_supported_version(2, {"client": glance_client_v2, "version": 2})
except ImportError:
    pass

try:
    from glanceclient.v1 import client as glance_client_v1
    VERSIONS.load_supported_version(1, {"client": glance_client_v1, "version": 1})
except ImportError:
    pass
...
class Image(base.APIResourceWrapper):
    _attrs = {
        "architecture", "container_format", "disk_format", "created_at", "owner", "size", "id", "status", "updated_at",
        "checksum", "visibility", "name", "is_public", "protected", "min_disk", "min_ram"
    }
    _ext_attrs = {"file", "locations", "schema", "tags", "virtual_size", "kernel_id", "ramdisk_id", "image_url"}
    ...
    def __getattribute__(self, attr):
        ...
        if VERSIONS.active >= 2 and attr == "properties":
            return {k: v for (k, v) in self._apiresource.items() if self.property_visible(k)}
        try:
    ...
    def is_public(self):
        ...
        return (getattr(self._apiresource, 'is_public', None)
                or getattr(self._apiresource, 'visibility', None) == "public")
    ...
    def to_dict(self, show_ext_attrs=False):
        ...
        image_dict['properties'] = {
            k: self._apiresource[k]
            for k in self._apiresource if self.property_visible(k, show_ext_attrs=show_ext_attrs)
        }
        return image_dict
...
PUBLIC_TO_VISIBILITY_MAP = {None: None, True: 'public', False: 'private'}
...
def _normalize_is_public_filter(filters):
    ...
    if VERSIONS.active >= 2:
        ...
        filters['is_public'] = (getattr(filters, 'visibility', None) == "public")
...
def _normalize_list_input(filters, **kwargs):
    ...
    if VERSIONS.active < 2:
        ...
        # }
        v1_keywords = [
            'page_size', 'limit', 'sort_dir', 'sort_key', 'marker', 'is_public', 'return_req_id', 'paginate'
        ]
...
def image_list_detailed(request,
    ...
    images_iter = glanceclient(request).images.list(page_size=request_size, limit=limit, **kwargs)
    has_prev_data = False
    has_more_data = False
    if paginate:
        ...
        if reversed_order:
            images = sorted(
                images, key=lambda image: (getattr(image, sort_key) or '').lower(), reverse=(sort_dir == 'desc'))
    else:
...
def image_update(request, image_id, **kwargs):
    ...
    try:
        ...
        return Image(glanceclient(request).images.update(image_id, **kwargs))
    finally:
        if image_data:
            try:
                ...
                LOG.warning('Failed to remove temporary image file ' '%(file)s (%(e)s)', {'file': filename, 'e': e})
...
def get_image_upload_mode():
    ...
    if mode not in ('off', 'legacy', 'direct'):
        LOG.warning(
            'HORIZON_IMAGES_UPLOAD_MODE has an unrecognized value of '
...
class ExternallyUploadedImage(Image):
    ...
    def to_dict(self):
        ...
        base_dict.update({'upload_url': self._url, 'token_id': self._token_id})
        return base_dict
...
class BaseGlanceMetadefAPIResourceWrapper(base.APIResourceWrapper):
    ...
    def description(self):
        return (getattr(self._apiresource, 'description', None) or getattr(self._apiresource, 'display_name', None))
...
class Namespace(BaseGlanceMetadefAPIResourceWrapper):
    ...
    _attrs = [
        'namespace', 'display_name', 'description', 'resource_type_associations', 'visibility', 'protected',
        'created_at', 'updated_at', 'properties', 'objects'
    ]

    @property
    def resource_type_names(self):
        result = [resource_type['name'] for resource_type in getattr(self._apiresource, 'resource_type_associations')]
...
@profiler.trace
def metadefs_namespace_list(request, filters=None, sort_dir='asc', sort_key='namespace', marker=None, paginate=False):
    ...
    namespaces_iter = glanceclient(request, '2').metadefs_namespace.list(page_size=request_size, limit=limit, **kwargs)
    ...
    if resource_types and properties_target:
        namespaces_iter = filter_properties_target(namespaces_iter, resource_types, properties_target)
...
@profiler.trace
def metadefs_namespace_full_list(request, resource_type, filters=None, *args, **kwargs):
    filters = filters or {}
    filters['resource_types'] = [resource_type]
    namespaces, has_more_data, has_prev_data = metadefs_namespace_list(request, filters, *args, **kwargs)
    return [metadefs_namespace_get(request, x.namespace, resource_type)
            for x in namespaces], has_more_data, has_prev_data
...
def metadefs_namespace_update(request, namespace_name, **properties):
    return glanceclient(request, '2').metadefs_namespace.update(namespace_name, **properties)
...
def metadefs_namespace_resource_types(request, namespace_name):
    resource_types = glanceclient(request, '2').metadefs_resource_type.get(namespace_name)
...
@profiler.trace
def metadefs_namespace_add_resource_type(request, namespace_name, resource_type):
    return glanceclient(request, '2').metadefs_resource_type.associate(namespace_name, **resource_type)


@profiler.trace
def metadefs_namespace_remove_resource_type(request, namespace_name, resource_type_name):
    glanceclient(request, '2').metadefs_resource_type.deassociate(namespace_name, resource_type_name)
