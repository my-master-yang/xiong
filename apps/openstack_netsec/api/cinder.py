...
from openstack_netsec.horizon.utils import functions as utils
from openstack_netsec.horizon.utils.memoized import memoized
from openstack_netsec.horizon.utils.memoized import memoized_with_request

from openstack_netsec.api import base
from openstack_netsec.api import microversions
from openstack_netsec.api import nova
from openstack_netsec.contrib import api as profiler
...
try:
    ...
    VERSIONS.load_supported_version('2', {"client": cinder_client_v2, "version": '2'})
except ImportError:
    pass
...
class BaseCinderAPIResourceWrapper(base.APIResourceWrapper):
    ...
    def name(self):
        ...
        return (getattr(self._apiresource, 'name', None) or getattr(self._apiresource, 'display_name', None)
                or getattr(self._apiresource, 'id', None))

    @property
    def description(self):
        return (getattr(self._apiresource, 'description', None)
                or getattr(self._apiresource, 'display_description', None))


class Volume(BaseCinderAPIResourceWrapper):

    _attrs = [
        'id', 'name', 'description', 'size', 'status', 'created_at', 'volume_type', 'availability_zone', 'imageRef',
        'bootable', 'snapshot_id', 'source_volid', 'attachments', 'tenant_name', 'consistencygroup_id',
        'os-vol-host-attr:host', 'os-vol-tenant-attr:tenant_id', 'metadata', 'volume_image_metadata', 'encrypted',
        'transfer'
    ]
...
class VolumeSnapshot(BaseCinderAPIResourceWrapper):
    ...
    _attrs = [
        'id', 'name', 'description', 'size', 'status', 'created_at', 'volume_id',
        'os-extended-snapshot-attributes:project_id', 'metadata'
    ]
...
class VolumeType(BaseCinderAPIResourceWrapper):
    ...
    _attrs = [
        'id', 'name', 'extra_specs', 'created_at', 'encryption', 'associated_qos_spec', 'description',
...
class VolumeConsistencyGroup(BaseCinderAPIResourceWrapper):
    ...
    _attrs = ['id', 'name', 'description', 'status', 'availability_zone', 'created_at', 'volume_types']


class VolumeCGSnapshot(BaseCinderAPIResourceWrapper):

    _attrs = ['id', 'name', 'description', 'status', 'created_at', 'consistencygroup_id']


class VolumeBackup(BaseCinderAPIResourceWrapper):

    _attrs = [
        'id', 'name', 'description', 'container', 'size', 'status', 'created_at', 'volume_id', 'availability_zone'
    ]
    _volume = None
...
class VolumePool(base.APIResourceWrapper):
    ...
    _attrs = [
        'name', 'pool_name', 'total_capacity_gb', 'free_capacity_gb', 'allocated_capacity_gb', 'QoS_support',
        'reserved_percentage', 'volume_backend_name', 'vendor_name', 'driver_version', 'storage_protocol',
        'extra_specs'
    ]
...
def get_auth_params_from_request(request):
    ...
    for service_name in ('volumev3', 'volumev2', 'volume'):
        try:
            ...
            pass
    if not cinder_urls:
        raise exceptions.ServiceCatalogException("no volume service configured")
...
def cinderclient(request_auth_params, version=None):
    ...
    for name, cinder_url in cinder_urls:
        ...
        raise exceptions.ServiceCatalogException("Cinder {version} requested but no '{service}' service "
...
def volume_list(request, search_opts=None, marker=None, sort_dir="desc"):
    volumes, _, __ = volume_list_paged(
        request, search_opts=search_opts, marker=marker, paginate=False, sort_dir=sort_dir)
    return volumes
...
@profiler.trace
def volume_list_paged(request, search_opts=None, marker=None, paginate=False, sort_dir="desc"):
...
def volume_retype(request, volume_id, new_type, migration_policy):
    return cinderclient(request).volumes.retype(volume_id, new_type, migration_policy)


@profiler.trace
def volume_set_bootable(request, volume_id, bootable):
    return cinderclient(request).volumes.set_bootable(volume_id, bootable)


@profiler.trace
def volume_update(request, volume_id, name, description):
    vol_data = {'name': name, 'description': description}
    vol_data = _replace_v2_parameters(vol_data)
    return cinderclient(request).volumes.update(volume_id, **vol_data)
...
@profiler.trace
def volume_upload_to_image(request, volume_id, force, image_name, container_format, disk_format):
    return cinderclient(request).volumes.upload_to_image(volume_id, force, image_name, container_format, disk_format)
...
@profiler.trace
def volume_migrate(request, volume_id, host, force_host_copy=False, lock_volume=False):
    return cinderclient(request).volumes.migrate_volume(volume_id, host, force_host_copy, lock_volume)
...
def volume_snapshot_list(request, search_opts=None):
    snapshots, _, __ = volume_snapshot_list_paged(request, search_opts=search_opts, paginate=False)
    return snapshots


@profiler.trace
def volume_snapshot_list_paged(request, search_opts=None, marker=None, paginate=False, sort_dir="desc"):
    has_more_data = False
    has_prev_data = False
    snapshots = []
    c_client = cinderclient(request)
    ...
    if VERSIONS.active > 1 and paginate:
        ...
        # by default, it is "created_at"
        sort = 'created_at:' + sort_dir
        for s in c_client.volume_snapshots.list(
                search_opts=search_opts, limit=page_size + 1, marker=marker, sort=sort):
            snapshots.append(VolumeSnapshot(s))

        snapshots, has_more_data, has_prev_data = update_pagination(snapshots, page_size, marker, sort_dir)
    else:
...
@profiler.trace
def volume_snapshot_create(request, volume_id, name, description=None, force=False):
    data = {'name': name, 'description': description, 'force': force}
    data = _replace_v2_parameters(data)

    return VolumeSnapshot(cinderclient(request).volume_snapshots.create(volume_id, **data))
...
def volume_snapshot_update(request, snapshot_id, name, description):
    snapshot_data = {'name': name, 'description': description}
    snapshot_data = _replace_v2_parameters(snapshot_data)
    return cinderclient(request).volume_snapshots.update(snapshot_id, **snapshot_data)


@profiler.trace
def volume_snapshot_set_metadata(request, snapshot_id, metadata):
    return cinderclient(request).volume_snapshots.set_metadata(snapshot_id, metadata)


@profiler.trace
def volume_snapshot_delete_metadata(request, snapshot_id, keys):
    return cinderclient(request).volume_snapshots.delete_metadata(snapshot_id, keys)


@profiler.trace
def volume_snapshot_reset_state(request, snapshot_id, state):
    return cinderclient(request).volume_snapshots.reset_state(snapshot_id, state)
...
def volume_cgroup_list(request, search_opts=None):
    ...
    if c_client is None:
        return []
    return [VolumeConsistencyGroup(s) for s in c_client.consistencygroups.list(search_opts=search_opts)]
...
@profiler.trace
def volume_cgroup_create(request, volume_types, name, description=None, availability_zone=None):
    data = {'name': name, 'description': description, 'availability_zone': availability_zone}

    cgroup = cinderclient(request).consistencygroups.create(volume_types, **data)
    return VolumeConsistencyGroup(cgroup)


@profiler.trace
def volume_cgroup_create_from_source(request,
                                     name,
                                     cg_snapshot_id=None,
    ...
    return VolumeConsistencyGroup(
        cinderclient(request).consistencygroups.create_from_src(cg_snapshot_id, source_cgroup_id, name, description,
                                                                user_id, project_id))
...
@profiler.trace
def volume_cgroup_update(request, cgroup_id, name=None, description=None, add_vols=None, remove_vols=None):
    cgroup_data = {}
    if name:
    ...
    return cinderclient(request).consistencygroups.update(cgroup_id, **cgroup_data)


def volume_cg_snapshot_create(request, cgroup_id, name, description=None):
    return VolumeCGSnapshot(cinderclient(request).cgsnapshots.create(cgroup_id, name, description))
...
def volume_cg_snapshot_list(request, search_opts=None):
    ...
    if c_client is None:
        return []
    return [VolumeCGSnapshot(s) for s in c_client.cgsnapshots.list(search_opts=search_opts)]
...
@profiler.trace
def volume_backup_list_paged(request, marker=None, paginate=False, sort_dir="desc"):
    has_more_data = False
    has_prev_data = False
    backups = []
    ...
    if VERSIONS.active > 1 and paginate:
        ...
        # by default, it is "created_at"
        sort = 'created_at:' + sort_dir
        for b in c_client.backups.list(limit=page_size + 1, marker=marker, sort=sort):
            backups.append(VolumeBackup(b))

        backups, has_more_data, has_prev_data = update_pagination(backups, page_size, marker, sort_dir)
    else:
...
@profiler.trace
def volume_backup_create(request, volume_id, container_name, name, description, force=False):
...
def volume_backup_restore(request, backup_id, volume_id):
    return cinderclient(request).restores.restore(backup_id=backup_id, volume_id=volume_id)


@profiler.trace
def volume_manage(request, host, identifier, id_type, name, description, volume_type, availability_zone, metadata,
                  bootable):
...
def volume_type_create(request, name, description=None, is_public=True):
    return cinderclient(request).volume_types.create(name, description, is_public)


@profiler.trace
def volume_type_update(request, volume_type_id, name=None, description=None, is_public=None):
    return cinderclient(request).volume_types.update(volume_type_id, name, description, is_public)
...
def volume_type_delete(request, volume_type_id):
    try:
        ...
        raise exceptions.BadRequest(_("This volume type is used by one or more volumes."))
...
def volume_encryption_type_create(request, volume_type_id, data):
    return cinderclient(request).volume_encryption_types.create(volume_type_id, specs=data)
...
def volume_encryption_type_update(request, volume_type_id, data):
    return cinderclient(request).volume_encryption_types.update(volume_type_id, specs=data)
...
def volume_type_extra_get(request, type_id, raw=False):
    ...
    if raw:
        return extras
    return [VolTypeExtraSpec(type_id, key, value) for key, value in extras.items()]
...
def qos_spec_get_keys(request, qos_spec_id, raw=False):
    ...
    if raw:
        return spec
    return [QosSpec(qos_spec_id, key, value) for key, value in qos_specs.items()]
...
def pool_list(request, detailed=False):
    ...
    return [VolumePool(v) for v in c_client.pools.list(detailed=detailed)]
...
def is_volume_service_enabled(request):
    return bool(
        base.is_service_enabled(request, 'volume') or base.is_service_enabled(request, 'volumev2')
        or base.is_service_enabled(request, 'volumev3'))
...
def volume_type_add_project_access(request, volume_type, project_id):
    return cinderclient(request).volume_type_access.add_project_access(volume_type, project_id)


def volume_type_remove_project_access(request, volume_type, project_id):
    return cinderclient(request).volume_type_access.remove_project_access(volume_type, project_id)
