...
def sort_flavor_list(request, flavors):
    ...
    def get_key(flavor, sort_key):
        try:
            ...
            LOG.warning('Could not find sort key "%s". Using the default ' '"ram" instead.', sort_key)
...
def availability_zone_list(request):
    ...
    try:
        ...
        exceptions.handle(request, _('Unable to retrieve Nova availability zones.'))
        return []
            ...
            if with_cidr:
                cidrs = (
                    [subnet.cidr for subnet in n['subnets']
                     if subnet.ip_version == 4] + [subnet.cidr for subnet in n['subnets'] if subnet.ip_version == 6])
                v += ' (%s)' % ', '.join(cidrs)
...
def port_field_data(request, with_network=False):
    ...
    def add_more_info_port_name(port, network):
        ...
        port_name = "{} ({})".format(port.name_or_id, ",".join([ip['ip_address'] for ip in port['fixed_ips']]))
    ...
    if api.base.is_service_enabled(request, 'network'):
        ...
        for network in network_list:
            ports.extend([(port.id, add_more_info_port_name(port, network))
                          for port in api.neutron.port_list_with_trunk_types(
                              request, network_id=network.id, tenant_id=request.user.tenant_id)
                          if (not port.device_owner and not isinstance(port, api.neutron.PortTrunkSubport))])
...
def server_group_field_data(request):
    ...
    if server_groups:
        ...
        return [
            ("", _("Select Server Group")),
        ] + server_groups_list

    return [
        ("", _("No server groups available")),
    ]
