...
def _get_key(args, kwargs, remove_callback):
    ...
    weak_kwargs = tuple(sorted((key, _try_weakref(value, remove_callback)) for (key, value) in kwargs.items()))
...
def memoized(func):
    ...
    def wrapped(*args, **kwargs):
        ...
        try:
            ...
            # decorated function.
            warnings.warn("The key %r is not hashable and cannot be memoized." % (key, ), UnhashableKeyWarning, 2)
...
def memoized_with_request(request_func, request_index=0):
    ...
    See openstack_netsec.api.nova for a complete example.
    """
