...
def sort_js_files(js_files):
    ...
    * specs: spec files for testing.  They have extension of `.spec.js`.
    ...
    other_sources = [
        f for f in js_files if not f.endswith(MODULE_EXT) and not f.endswith(MOCK_EXT) and not f.endswith(SPEC_EXT)
    ]
...
def discover_static_files(base_path, sub_path=''):
    ...
    """
    js_files = discover_files(base_path, sub_path=sub_path, ext='.js', trim_base_path=True)
    sources, mocks, specs = sort_js_files(js_files)
    html_files = discover_files(base_path, sub_path=sub_path, ext='.html', trim_base_path=True)
...
def populate_horizon_config(horizon_config, base_path, sub_path='', prepend=False):
    sources, mocks, specs, template = discover_static_files(base_path, sub_path=sub_path)
    if prepend:
