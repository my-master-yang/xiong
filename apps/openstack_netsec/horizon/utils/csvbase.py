    ...
    def write_csv_header(self):
        if self.is_dict:
            try:
                ...
                # For Python<2.7
                self.writer.writerow(dict(zip(self.writer.fieldnames, self.writer.fieldnames)))

    def write_csv_row(self, args):
        if self.is_dict:
            self.writer.writerow(dict(zip(self.writer.fieldnames, [self.encode(col) for col in args])))
        else:
...
class BaseCsvResponse(CsvDataMixin, HttpResponse):
    ...
    def __init__(self, request, template, context, content_type, **kwargs):
        ...
        self['Content-Disposition'] = 'attachment; filename="%s"' % (kwargs.get("filename", "export.csv"), )
