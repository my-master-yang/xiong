...
filter_regex = re.compile(r"""{\$\s*('([^']|\\')+'|"([^"]|\\")+")\s*\|\s*translate\s*\$}""")
...
HTML_ENTITY_DECODED = {'reg': u'®', 'times': u'×'}
...
class AngularGettextHTMLParser(html_parser.HTMLParser):
    ...
    def __init__(self):
        try:
            super(AngularGettextHTMLParser, self).__init__(convert_charrefs=False)
        except TypeError:
        ...
        if tag == 'translate' or \
            ...
            for attr in attrs:
                ...
                for match in filter_regex.findall(attr[1]):
                    if match:
                        self.strings.append((self.line, u'gettext', match[0][1:-1], []))
    ...
    def handle_endtag(self, tag):
        if self.in_translate:
            if len(self.inner_tags) > 0:
                ...
                return
            if self.plural_form:
                messages = (self.data.strip(), self.plural_form)
                func_name = u'ngettext'
            else:
                messages = self.data.strip()
                func_name = u'gettext'
            self.strings.append((self.line, func_name, messages, self.comments))
            self.in_translate = False
            self.data = ''
            self.comments = []
