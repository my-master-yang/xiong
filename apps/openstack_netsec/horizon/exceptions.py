...
from openstack_netsec.horizon.default import HORIZON_CONFIG
...
class GetFileError(HorizonException):
    ...
    def __repr__(self):
        return '<%s name=%r resource_type=%r>' % (self.__class__.__name__, self.attrs['name'],
...
RECOVERABLE = (AlreadyExists, Conflict, NotAvailable, ServiceCatalogException, BadRequest)
...
def handle_unauthorized(request, message, redirect, ignore, escalate, handled, force_silence, force_log, log_method,
                        log_entry, log_level):
    if ignore:
...
def handle_notfound(request, message, redirect, ignore, escalate, handled, force_silence, force_log, log_method,
                    log_entry, log_level):
...
def handle_recoverable(request, message, redirect, ignore, escalate, handled, force_silence, force_log, log_method,
                       log_entry, log_level):
    if not force_silence and not handled and (not ignore or force_log):
...
def handle(request, message=None, redirect=None, ignore=False, escalate=False, log_level=None, force_log=None):
    ...
    for exc_handler in HANDLE_EXC_METHODS:
        if issubclass(exc_type, exc_handler['exc']):
            ...
            ret = handler(request, user_message, redirect, ignore, exc_handler.get('escalate', escalate), handled,
                          force_silence, force_log, log_method, log_entry, log_level)
            if ret:
    ...
    if message:
        ret = handle_recoverable(request, user_message, redirect, ignore, escalate, handled, force_silence, force_log,
